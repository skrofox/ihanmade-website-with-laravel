...
@section('header', 'Sửa Stock Item')
@section('title', 'Sửa Stock Item')
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Sửa Stock Item</h1>
            <p class="text-gray-600 mt-2">Cập nhật thông tin stock item</p>
        </div>

        <div class="bg-white rounded-lg border border-gray-200 p-6">
            <form action="{{ route('stock_update', $stockItem->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="ware_house_id" id="ware_house_id" required
                        ...
                        @foreach($warehouses as $warehouse)
                            <option value="{{ $warehouse->id }}" {{ old('ware_house_id', $stockItem->ware_house_id) == $warehouse->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="variant_id" id="variant_id" required
                        ...
                        @foreach($variants as $variant)
                            <option value="{{ $variant->id }}" {{ old('variant_id', $stockItem->variant_id) == $variant->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="on_hand" id="on_hand" min="0" required
                           value="{{ old('on_hand', $stockItem->on_hand) }}"
                ...
                <div>
                    ...
                    <input type="number" name="reserved" id="reserved" min="0"
                           value="{{ old('reserved', $stockItem->reserved) }}"
                ...
                <div>
                    ...
                    <input type="number" name="min_stock_level" id="min_stock_level" min="0"
                           value="{{ old('min_stock_level', $stockItem->min_stock_level) }}"
                ...
                <div>
                    ...
                    <input type="number" name="max_stock_level" id="max_stock_level" min="0"
                           value="{{ old('max_stock_level', $stockItem->max_stock_level) }}"
